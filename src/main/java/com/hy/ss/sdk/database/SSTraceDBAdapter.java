...
public class SSTraceDBAdapter {
	private static final String TAG = (SSTraceDBAdapter.class.getSimpleName());
	...
	private static final String SQL_DATA_CREATE = String.format(DATA_CREATE,
			TABLE_TRACE, FIELD_ROWID, FIELD_X_COORD, FIELD_Y_COORD,
	...
	public SSTraceDBAdapter(Context ctx) {
		this.context = ctx;
		dbFile = new File(SSFileManager.getTraceDBPath());
	...
	public boolean deleteTrace(long tag) {
		...
		return db.delete(TABLE_TRACE, whereClause, whereArgs) > 0;
	}
	...
	public boolean eraseTraceTable() {
		return db.delete(TABLE_TRACE, null, null) > 0;
	}
	...
	private boolean insertTracePoint(double x, double y, int floor, double mx,
		...
		return db.insert(TABLE_TRACE, null, values) > 0;
	}
	...
	public List<SSTrace> getAllTraces() {
		...
		Cursor cursor = db.query(true, TABLE_TRACE, columns, null, null, null,
	...
	public List<SSTrace> getAllTraces(String marketID) {
		...
		Cursor cursor = db.query(true, TABLE_TRACE, columns, selection,
				selectionArgs, null, null, orderBy, null, null);
	...
	public List<Integer> getAllTags(String marketID) {
		...
		String[] selectionArgs = new String[] { marketID };
		Cursor cursor = db.query(true, TABLE_TRACE, columns, selction,
